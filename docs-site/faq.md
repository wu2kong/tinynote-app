...
打开「设置 → 同步」，选择 Git 同步并添加同步源。TinyNote 会在应用内完成初始化和授权，不需要安装 Git 软件，也不需要使用命令行。网页端只支持 HTTPS + 令牌。
...
应用内删除通常不可撤销。若已开启 Git 同步，可用同步页的「撤销变更」。若有 zip 备份，按 [备份说明](/backup#恢复) 恢复。
